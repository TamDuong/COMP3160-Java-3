...
public class SortingAlgorithms
{
    ...
    // Bubble sort
    // The best case of this bubble sort is O(n) which is when the list
    // is already sorted. Which mean it just run through the list without swapping any element 
    public static void bubbleSort(int[] a)
    {
        ...
        while (notSorted){
        	...
        	for (int i = 0; i < a.length - larIndices; i++){
        		if (a[i] > a[i + 1]) {
        		    int temp = a[i];
        		    a[i] = a[i + 1];
        		    a[i + 1] = temp;
